...
수정 사항:
1. ThreadPoolExecutor shutdown(wait=False) — hang 방지
2. _safe_result 에 timeout 파라미터 추가
3. _keywords() — site_content + sim_results competitor_mentions 활용해 실제 블루오션 키워드 발굴
4. _STRATEGY_VERSION v5 로 bump (캐시 무효화)
5. sim_results 에서 경쟁 브랜드 추출 로직 추가
"""
...
사이트 내용을 직접 분석해서 구체적 문제점을 지적하세요. 각 항목은 완전한 문장으로 끊기지 않게. 번호 없이 한 줄씩:"""
...
[현재 AI 응답에서 자주 언급되는 경쟁 브랜드]
{comp_brands}

위 사이트의 실제 서비스·강점을 분석하여, ChatGPT·Gemini·Perplexity가 더 자주 인용할 블루오션 키워드 5개를 추천하세요.

조건:
- 이 사이트의 실제 서비스/강점과 직접 연관된 구체적 키워드 (추상적 키워드 금지)
- 위 경쟁 브랜드들이 아직 강하게 커버하지 못한 틈새 영역 우선
- {scope_inst}
- 롱테일(long-tail) 키워드 선호 (2~4 어절, 검색 의도 명확한 것)
- 한국어만 (영어 혼용 가능, 단 영어만 금지)
...
def run_strategy_analysis(
    ...
    # ── 사이트 크롤 (키워드·진단·GEO 가이드에 site_content 공급) ──
    site_content = ""
    if target_url:
    ...
    # ── sim_results 에서 경쟁 브랜드 집계 (블루오션 키워드 품질 향상) ──
    _sim_brands: list[str] = []
    if sim_results:
        _brand_counts: dict = {}
        for _r in sim_results:
            _comp_m = (
                _r.competitor_mentions if hasattr(_r, "competitor_mentions")
                else (_r.get("competitor_mentions", {}) if isinstance(_r, dict) else {})
            )
            for _brand, _data in _comp_m.items():
                _cnt = _data.get("mentions", 0) if isinstance(_data, dict) else 1
                _brand_counts[_brand] = _brand_counts.get(_brand, 0) + _cnt
        _sim_brands = sorted(_brand_counts, key=_brand_counts.get, reverse=True)[:5]
    ...
    def _diagnosis() -> list[str]:
        ...
        with CaptureError("strategy_diag", log_level="warning"):
            r = (
                call_gpt(client_gpt, prompt, system=STRATEGY_SYSTEM,
                         max_tokens=1200, model=model_gpt, temperature=0.4)
                if client_gpt else
                call_gemini(client_gemini, prompt, max_tokens=1200, temperature=0.4)
            )
        items = [d.strip().lstrip("•-*") for d in r.split("\n") if d.strip()][:3]
        return items or ["데이터 부족으로 분석 불가"]

    def _keywords() -> list[str]:
        comp_brands = ", ".join(_sim_brands) if _sim_brands else "미집계 (시뮬레이션 데이터 없음)"
    ...
    def _geo() -> list[str]:
        ...
        with CaptureError("strategy_geo", log_level="warning"):
            r = (
                call_gpt(client_gpt, prompt, system=STRATEGY_SYSTEM,
                         max_tokens=1500, model=model_gpt, temperature=0.5)
                if client_gpt else
                call_gemini(client_gemini, prompt, max_tokens=1500, temperature=0.5)
            )
        items = [g.strip() for g in re.split(r'\n(?=\d+\.)', r) if g.strip()][:3]
        return items or ["분석 중 오류"]

    def _safe_result(future, fallback, label: str = "", timeout: int = 60):
        try:
    ...
    # ── 4개 병렬 실행 — try/finally 로 shutdown(wait=False) 보장 ──
    _ex = concurrent.futures.ThreadPoolExecutor(max_workers=4)
    try:
        f_comp = _ex.submit(_competitors_fn)
        f_diag = _ex.submit(_diagnosis)
        f_kw   = _ex.submit(_keywords)
        f_geo  = _ex.submit(_geo)

        competitors_result = _safe_result(f_comp, [],                     "competitors")
